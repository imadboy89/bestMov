...
class API {
  constructor() {
    ...
    this.cats = [
      'trending',
      'movies',
      'movies/latest-bluray-2017-2016',
      'tv',
      'latest',
      'arab',
    ];
    this.pagenation =
      '?page=[page_nbr]&output_format=json&output_mode=movies_list';
      ...
      .then(responseJson => {
        ...
        for (let i = 0; i < this.configs['action_link'].length; i++) {
          if (this.configs['action_link'][i] == '') {
            ...
            this.configs['action_link'][i] = Base64.atob(
              this.configs['action_link'][i]
            );
          }
        }
        this.configs['links_manager'] =
          'action_using' in this.configs
            ? this.configs['action_link'][this.configs['action_using']]
            : this.configs['action_link'][0];
        this.setConfigs();
      })
      .catch(error => {
